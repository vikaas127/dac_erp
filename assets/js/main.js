...
function calculate_total() {
  ...
  const basicDiscountPercent = parseFloat($('#basic_discount_percent').val()) || 0;
  const basicDiscountType = $('#basic_discount_type').val();
  let basicDiscountAmount = basicDiscountType === 'percent'
    ? (subtotal * basicDiscountPercent) / 100
    : basicDiscountPercent;
  $('.basic-discount-amount').text('-' + format_money(basicDiscountAmount));
  subtotal -= basicDiscountAmount;

  // ✅ Apply quantity-based discount after basic
  let quantityDiscountPercent = 0;
  if (subtotal >= 400000) quantityDiscountPercent = 4;
  else if (subtotal >= 300000) quantityDiscountPercent = 3;
  else if (subtotal >= 200000) quantityDiscountPercent = 2;

  let quantityDiscountAmount = (subtotal * quantityDiscountPercent) / 100;
  $('#quantity_discount_display').text(quantityDiscountPercent + '% (-' + format_money(quantityDiscountAmount) + ')');
  subtotal -= quantityDiscountAmount;

  // ✅ Apply special discount
  const specialDiscountPercent = parseFloat($('#special_discount_percent').val()) || 0;
  const specialDiscountAmount = (subtotal * specialDiscountPercent) / 100;
  subtotal -= specialDiscountAmount;

  // ✅ Apply offer discount
  const offerDiscountPercent = parseFloat($('#offer_discount_percent').val()) || 0;
  const offerDiscountAmount = (subtotal * offerDiscountPercent) / 100;
  subtotal -= offerDiscountAmount;

  // At this point, subtotal is reduced by all discounts
  ...
  // Traditional discount (estimate default) after tax
  if (
    discount_percent !== "" &&
    discount_percent != 0 &&
...
$(document).on('input change', '#basic_discount_percent, #basic_discount_type, #special_discount_percent, #offer_discount_percent', function() {
  calculate_total();
});
