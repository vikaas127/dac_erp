...
function clear_item_preview_values(e){var t=$("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),a=$(".main");a.find("textarea").val(""),a.find('td.custom_field input[type="checkbox"]').prop("checked",!1),a.find("td.custom_field input:not(:checkbox):not(:hidden)").val(""),a.find("td.custom_field select").selectpicker("val",""),a.find('input[name="quantity"]').val(1),a.find("select.tax").selectpicker("val",t),a.find('input[name="rate"]').val(""),a.find('input[name="item_discount_percent"]').val(""),a.find('input[name="item_original_discount_percent"]').val(""),a.find('input[name="unit"]').val(""),$('input[name="task_id"]').val(""),$('input[name="expense_id"]').val("")}
function calculate_total(){if($("body").hasClass("no-calculate-total")){console.log("⛔ Calculation skipped");return!1}
var rows=$(".table.has-calculations tbody tr.item");var subtotal=0;var totalBeforeQty=0;var totalAfterQty=0;var specialDiscountTotal=0;var offerDiscountTotal=0;var quantityDiscountTotal=0;var specialPercent=parseFloat($("#special_discount_percent").val())||0;var offerPercent=parseFloat($("#offer_discount_percent").val())||0;var adjustment=parseFloat($('input[name="adjustment"]').val())||0;console.log("=== START CALCULATION ===");console.log("Global Discounts:",{specialPercent,offerPercent,adjustment});$(".tax-area").remove();rows.each(function(index){var row=$(this);var qty=parseFloat(row.find("[data-quantity]").val())||1;var rate=parseFloat(row.find("td.rate input").val())||0;var base=qty*rate;subtotal+=base;var originalPercent=parseFloat(row.find('input[name$="[item_original_discount_percent]"]').val())||0;var afterOriginal=base*(1-originalPercent/100);var specialAmt=afterOriginal*(specialPercent/100);var afterSpecial=afterOriginal-specialAmt;var offerAmt=afterSpecial*(offerPercent/100);var afterOffer=afterSpecial-offerAmt;specialDiscountTotal+=specialAmt;offerDiscountTotal+=offerAmt;totalBeforeQty+=afterOffer;row.data("base",base);row.data("amountBeforeQty",afterOffer);console.log(`ROW ${index + 1} PRE-QTY`,{base,originalPercent,afterOriginal,specialAmt,offerAmt,afterOffer})});var quantityPercent=0;if($("#quantity_discount_allowed").val()==="1"){if(totalBeforeQty>416800)quantityPercent=4;else if(totalBeforeQty>309300)quantityPercent=3;else if(totalBeforeQty>204100)quantityPercent=2}
quantityDiscountTotal=totalBeforeQty*(quantityPercent/100);totalAfterQty=totalBeforeQty-quantityDiscountTotal+adjustment;console.log("QUANTITY DISCOUNT SUMMARY",{totalBeforeQty,quantityPercent,quantityDiscountTotal,totalAfterQty});rows.each(function(index){var row=$(this);var base=row.data("base");var amountBeforeQty=row.data("amountBeforeQty");var shareRatio=totalBeforeQty>0?amountBeforeQty/totalBeforeQty:0;var qtyShare=quantityDiscountTotal*shareRatio;var finalAmount=amountBeforeQty-qtyShare;row.find("td.amount").html(format_money(finalAmount,!0));var effectivePercent=base>0?100*(1-finalAmount/base):0;row.find('input[name$="[item_discount_percent]"]').val(accounting.toFixed(effectivePercent,app.options.decimal_places));var qty=parseFloat(row.find("[data-quantity]").val())||1;var unitPrice=qty>0?finalAmount/qty:0;row.find('input[name$="[item_price]"]').val(accounting.toFixed(unitPrice,app.options.decimal_places));console.log(`ROW ${index + 1} POST-QTY`,{base,amountBeforeQty,qtyShare,finalAmount,effectivePercent})});$(".subtotal").html(format_money(subtotal,!0)+hidden_input("subtotal",accounting.toFixed(subtotal,app.options.decimal_places)));$(".total").html(format_money(totalAfterQty,!0)+hidden_input("total",accounting.toFixed(totalAfterQty,app.options.decimal_places)));$("#special_discount_area .discount-total").html("-"+format_money(specialDiscountTotal||0,!0));$("#offer_discount_area .discount-total").html("-"+format_money(offerDiscountTotal||0,!0));$("#special_discount_total").val(accounting.toFixed(specialDiscountTotal||0,app.options.decimal_places));$("#offer_discount_total").val(accounting.toFixed(offerDiscountTotal||0,app.options.decimal_places));$("#quantity_discount_percent").val(quantityPercent);$("#quantity_discount_total").val(accounting.toFixed(quantityDiscountTotal||0,app.options.decimal_places));$("#quantity_discount_area .discount-total").html("-"+format_money(quantityDiscountTotal||0,!0));$(".adjustment").html(format_money(adjustment));console.log("FINAL TOTALS",{subtotal,specialDiscountTotal,offerDiscountTotal,quantityDiscountTotal,adjustment,totalAfterQty});console.log("=== END CALCULATION ===");$(document).trigger("sales-total-calculated")}
