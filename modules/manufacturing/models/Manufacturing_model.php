...
class Manufacturing_model extends App_Model
{
	...
	public function get_distinct_operation_names()
	{
		$this->db->distinct();
		$this->db->select('operation_name');
		$this->db->from(db_prefix() . 'mrp_work_orders');
		$this->db->where('operation_name IS NOT NULL'); // Optional: avoid nulls
		$this->db->where('operation_name !=', '');      // Optional: avoid empty strings

		$query = $this->db->get();
		$operation_names = [];

		foreach ($query->result_array() as $row) {
			$operation_names[] = $row['operation_name'];
		}
...
public function get_inventory_options($commodity_id)
{
    $this->db->where('commodity_id', $commodity_id);
    $inventory = $this->db->get(db_prefix() . 'inventory_manage')->result();
    ...
    foreach ($inventory as $row) {
        ...
        if (!isset($grouped[$wid])) {
            // Get names from reference tables
            $warehouse_name = $this->db->get_where(db_prefix().'warehouse', ['warehouse_id' => $wid])->row()->warehouse_name ?? '';
            $grouped[$wid] = [
                'id' => $wid,
        ...
        // Append lot/rack/shelf if not already added
        if ($row->lot_id && !in_array($row->lot_id, array_column($grouped[$wid]['lots'], 'id'))) {
            $lot_name = $this->db->get_where(db_prefix().'wr_lot', ['lot_id' => $row->lot_id])->row()->lot_name ?? '';
            $grouped[$wid]['lots'][] = ['id' => $row->lot_id, 'label' => $lot_name];
        }

        if ($row->rack_id && !in_array($row->rack_id, array_column($grouped[$wid]['racks'], 'id'))) {
            $rack_name = $this->db->get_where(db_prefix().'wr_rack', ['rack_id' => $row->rack_id])->row()->rack_name ?? '';
            $grouped[$wid]['racks'][] = ['id' => $row->rack_id, 'label' => $rack_name];
        }

        if ($row->shelf_id && !in_array($row->shelf_id, array_column($grouped[$wid]['shelves'], 'id'))) {
            $shelf_name = $this->db->get_where(db_prefix().'wr_shelf', ['shelf_id' => $row->shelf_id])->row()->shelf_name ?? '';
            $grouped[$wid]['shelves'][] = ['id' => $row->shelf_id, 'label' => $shelf_name];
        }
    }

    return array_values($grouped); // return as list
}

private function get_id_by_name($table, $id_column, $name_column, $name_value) {
    $row = $this->db->get_where(db_prefix() . $table, [$name_column => $name_value])->row();
    return $row ? $row->$id_column : null;
}
...
public function get_product_for_scrap()
	...
	public function add_manufacturing_order($data)
	{
		...
		if(count($es_detail) > 0){
			foreach ($es_detail as $key => $row) {
				// Convert warehouse name to ID
				if (!empty($row['warehouse_id']) && !is_numeric($row['warehouse_id'])) {
					$es_detail[$key]['warehouse_id'] = $this->get_id_by_name('warehouse', 'warehouse_id', 'warehouse_name', $row['warehouse_id']);
				}

				// Convert lot name to ID
				if (!empty($row['lot_id']) && !is_numeric($row['lot_id'])) {
					$es_detail[$key]['lot_id'] = $this->get_id_by_name('wr_lot', 'lot_id', 'lot_name', $row['lot_id']);
				}

				// Convert rack name to ID
				if (!empty($row['rack_id']) && !is_numeric($row['rack_id'])) {
					$es_detail[$key]['rack_id'] = $this->get_id_by_name('wr_rack', 'rack_id', 'rack_name', $row['rack_id']);
				}

				// Convert shelf name to ID
				if (!empty($row['shelf_id']) && !is_numeric($row['shelf_id'])) {
					$es_detail[$key]['shelf_id'] = $this->get_id_by_name('wr_shelf', 'shelf_id', 'shelf_name', $row['shelf_id']);
				}
			}
			...
			if (isset($insert_id)) {
				...
				log_message('debug', 'Inserted manufacturing order detail: ' . print_r($es_detail, true));
