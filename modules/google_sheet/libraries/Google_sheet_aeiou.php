...
class Google_sheet_aeiou
{
    public static function getPurchaseData($code)
    {
		$givemecode = Google_sheet_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Google_sheet_Requests::get($verify_url.'?code='.$code, $headers, $options);
        ...
        if (4 == count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Google_sheet_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
