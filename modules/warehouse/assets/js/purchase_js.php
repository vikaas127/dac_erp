...
$('select[name="pr_order_id"]').on('change', function() {
	...
	$.get(admin_url+'warehouse/coppy_pur_request/'+pr_order_id + '/' + warehouse_id_m).done(function(response){
		...
		if(response){
			...
			if (typeof response.shipping_fee !== 'undefined' && response.shipping_fee !== null) {
				$('input[name="ShippingCharge"]').val(response.shipping_fee);
			} else {
				$('input[name="ShippingCharge"]').val('');
			}
...
function purchase_calculate_total() {
    wh_calculate_total();
}
...
function wh_add_item_to_preview(id) {
	"use strict";
    console.log('wh_add_item_to_preview called with id:', id);
...
function wh_add_item_to_table(data, itemid) {
	"use strict";
    console.log('wh_add_item_to_table called with data:', data, 'itemid:', itemid);
...
function wh_calculate_total(){
	...
	var shipping_charge = parseFloat($('input[name="ShippingCharge"]').val());
    if (isNaN(shipping_charge)) {
        shipping_charge = 0;
    }
    $('.ShippingCharge').html(format_money(shipping_charge));
