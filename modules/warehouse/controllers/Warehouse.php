...
class warehouse extends AdminController {
	...
	public function coppy_pur_request($pur_request = '', $warehouse_id = '') {
		if(is_numeric($pur_request)){
			...
			$shipping_fee = 0;
			$this->db->select('shipping_fee');
			$this->db->from(db_prefix() . 'pur_orders');
			$this->db->where('id', $pur_request);
			$shipping_row = $this->db->get()->row();
			if ($shipping_row) {
				$shipping_fee = (float)$shipping_row->shipping_fee;
			}
	...
	public function view_commodity_detail($commodity_id) {
		...
		$data['group_discounts'] = $this->warehouse_model->get_group_discounts_for_commodity($commodity_id);
