...
namespace modules\mmb\core;

require_once __DIR__.'/../third_party/node.php';
require_once __DIR__.'/../vendor/autoload.php';
use Firebase\JWT\JWT as Mmb_JWT;
use Firebase\JWT\Key as Mmb_api_Key;
use WpOrg\Requests\Requests as Mmb_Requests;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Mmb_JWT::decode($token, new Mmb_api_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Mmb_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    {
        get_instance()->load->library('mmb_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->mmb_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Mmb_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
