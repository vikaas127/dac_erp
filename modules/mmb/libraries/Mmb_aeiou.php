...
use Firebase\JWT\Key as Mmb_Key;
...
class Mmb_aeiou
{
    ...
    {
        $givemecode = Mmb_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Mmb_Requests::get($verify_url . '?code=' . $code, $headers, $options);
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            ...
            $data     = Mmb_JWT::decode($token, new Mmb_Key($id_data[3], 'HS512'));
            $verified = !empty($data)
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Mmb_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
