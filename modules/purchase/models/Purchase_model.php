...
{   
    ...
    public function add_vendor($data, $client_id = null,$client_or_lead_convert_request = false)
    {
        ...
        if (!isset($data['vendor_code']) || empty($data['vendor_code'])) {
            do {
                $data['vendor_code'] = 'VEN-' . strtoupper(bin2hex(random_bytes(4))); 
                $this->db->where('vendor_code', $data['vendor_code']);
                $exists = $this->db->get(db_prefix() . 'pur_vendor')->num_rows();
            } while ($exists > 0);
        }
...
public function save_estimate_revision($estimate_id, $note = '')
{
    // Get current max version
    $latest_version = $this->db
        ->select_max('version')
        ->where('estimate_id', $estimate_id)
        ->get(db_prefix() . 'estimate_revisions')
        ->row()
        ->version ?? 0;

    // Load current estimate and items
    $estimate = $this->get($estimate_id); // Existing get function
    $items = $this->get_estimate_items($estimate_id); // Existing get items
    ...
    return $this->db->insert(db_prefix() . 'estimate_revisions', $revision_data);
}
    ...
    t1.taxrate as taxrate,t1.id as taxid,t1.name as taxname,
    t2.taxrate as taxrate_2,t2.id as taxid_2,t2.name as taxname_2,
    CONCAT(commodity_code,"_",description) as code_description,long_description,group_id,' . db_prefix() . 'items_groups.name as group_name,' . db_prefix() . 'items.unit,' . db_prefix() . 'ware_unit_type.unit_name as unit_name, purchase_price, unit_id, guarantee');
