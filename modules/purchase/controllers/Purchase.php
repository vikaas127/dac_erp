...
/**
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        for ($row = 1; $row < count($data); $row++) {
                            ...
                            // $value_vendor_code    = isset($data[$row][0]) ? $data[$row][0] : '' ;
                               $value_fist_name    = isset($data[$row][0]) ? $data[$row][0] : '' ;
                                $value_last_name    = isset($data[$row][1]) ? $data[$row][1] : '';
                                $value_email   = isset($data[$row][2]) ? $data[$row][2] : '' ;
                                $value_contact_phonenumber      = isset($data[$row][3]) ? $data[$row][3] : '';
                                $value_position       = isset($data[$row][4]) ? $data[$row][4] : '';
                                $value_company            = isset($data[$row][5]) ? $data[$row][5] : '';
                                $value_vat            = isset($data[$row][6]) ? $data[$row][6] : '';
                                $value_phonenumber            = isset($data[$row][7]) ? $data[$row][7] : '';
                                $value_country            = isset($data[$row][8]) ? $data[$row][8] : '';
                                $value_city            = isset($data[$row][9]) ? $data[$row][9] : '';
                                $value_zip            = isset($data[$row][10]) ? $data[$row][10] : '';
                                $value_state            = isset($data[$row][11]) ? $data[$row][11] : '';
                                $value_address            = isset($data[$row][12]) ? $data[$row][12] : '';
                                $value_website            = isset($data[$row][13]) ? $data[$row][13] : '';
                                $value_bank_detail            = isset($data[$row][14]) ? $data[$row][14] : '';
                                $value_payment_terms            = isset($data[$row][15]) ? $data[$row][15] : '';
                                $value_pur_billing_street            = isset($data[$row][16]) ? $data[$row][16] : '';
                                $value_pur_billing_city            = isset($data[$row][17]) ? $data[$row][17] : '';
                                $value_pur_billing_state            = isset($data[$row][18]) ? $data[$row][18] : '';
                                $value_pur_billing_zip            = isset($data[$row][19]) ? $data[$row][19] : '';
                                $value_pur_billing_country            = isset($data[$row][20]) ? $data[$row][20] : '';
                                $value_pur_shipping_street            = isset($data[$row][21]) ? $data[$row][21] : '';
                                $value_pur_shipping_city            = isset($data[$row][22]) ? $data[$row][22] : '';
                                $value_pur_shipping_state            = isset($data[$row][23]) ? $data[$row][23] : '';
                                $value_pur_shipping_zip            = isset($data[$row][24]) ? $data[$row][24] : '';
                                $value_pur_shipping_country            = isset($data[$row][25]) ? $data[$row][25] : '';

                                $vendor_code = $this->generate_unique_vendor_code();
    ...
    private function generate_unique_vendor_code() {
        do {
            $vendor_code = 'VEN' . time() . rand(1000, 9999);
            $this->db->where('vendor_code', $vendor_code);
            $exists = $this->db->count_all_results(db_prefix().'pur_vendor');
        } while ($exists > 0);
     ...
     public function revise($id)
{
    ...
    $success = $this->estimates_model->save_estimate_revision($id, $note);

    if ($success) {
        set_alert('success', _l('estimate_revised_successfully'));
    } else {
        set_alert('warning', _l('estimate_revision_failed'));
    }
...
{
    log_message('info', '[get_item_by_id] Request received. ID: ' . $id . ', Currency Rate: ' . $currency_rate);

    if ($this->input->is_ajax_request()) {
        log_message('info', '[get_item_by_id] AJAX request confirmed.');

        $item = $this->purchase_model->get_item_v2($id);
        log_message('debug', '[get_item_by_id] Item retrieved: ' . print_r($item, true));
        ...
        if ($currency_rate != 1) {
            ...
            log_message('info', "[get_item_by_id] Currency adjusted: Original = {$original_price}, Adjusted = {$item->purchase_price}");
        }

        $html = '<option value=""></option>';
        $item->warehouses_html = $html;

        log_message('debug', '[get_item_by_id] Final item object: ' . print_r($item, true));

        echo json_encode($item);
    } else {
        log_message('error', '[get_item_by_id] Non-AJAX request rejected.');
        show_error('Invalid request type', 400);
    }
}
