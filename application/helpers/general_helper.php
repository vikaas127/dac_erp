...
function to_sql_date($date, $datetime = false)
{
    ...
    if ($datetime == false) {
        ...
        if (preg_match('/^(\d{4})-(\d{1,2})-(\d{1,2})$/', $date)) {
         ...
         $date_object = DateTime::createFromFormat($from_format, $date);

if ($date_object) {
    // Format the date and apply filters
    return hooks()->apply_filters(
        'to_sql_date_formatted',
        $date_object->format($to_date)
    );
} else {
    // Log the error details for debugging
    error_log("Error: Date conversion failed. From format: $from_format, Date: $date");
