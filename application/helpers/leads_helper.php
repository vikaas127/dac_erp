...
function render_leads_nature_of_work_select($works, $selected = '', $name = 'nature_of_work', $lang_key = 'lead_nature_of_work')
{
    $select = '<div class="form-group">';
    $select .= '<label for="' . $name . '" class="control-label">' . _l($lang_key) . '</label>';

    $select .= '<select class="selectpicker" id="' . $name . '" data-width="100%" name="nature_of_work_id" data-live-search="true" data-none-selected-text="' . _l('dropdown_non_selected_tex') . '">';
    ...
    foreach ($works as $work) {
        $sel = ($selected == $work['id']) ? ' selected' : '';
        $select .= '<option value="' . $work['id'] . '"' . $sel . '>' . $work['name'] . '</option>';
    }
