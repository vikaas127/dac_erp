...
class Projects extends AdminController
{
    ...
    public function get_rel_project_data($id, $task_id = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($task_id) && $task_id !== 'undefined') {
                $task = $this->tasks_model->get($task_id);
            ...
            $milestone_options = '<option value=""></option>'; 
            $milestones = $this->projects_model->get_milestones($id);
            foreach ($milestones as $milestone) {
                $selected = ($milestone['id'] == $selected_milestone) ? ' selected' : '';
                $milestone_options .= '<option value="' . e($milestone['id']) . '"' . $selected . '>' . e($milestone['name']) . '</option>';
            }

            $assignee_options = '';
            $members = $this->projects_model->get_project_members($id, true);
            foreach ($members as $member) {
                $selected = in_array($member['staff_id'], $assigned) ? ' selected' : '';
                $name = $member['firstname'] . ' ' . $member['lastname'];
                $assignee_options .= '<option value="' . e($member['staff_id']) . '"' . $selected . '>' . e($name) . '</option>';
            }
