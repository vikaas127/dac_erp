...
class Estimate extends ClientsController
{
    public function index($id = '', $hash = '')
    {
        ...
        if ($estimate->status == 2) { // Status "sent"
    $revision_data = [
        'estimate_id'     => $id,
    ...
    $this->db->insert(db_prefix() . 'estimate_revisions', $revision_data);

    $revision_id = $this->db->insert_id();

    // Save old estimate items into revision items
    $estimate_items = $this->get_estimate_items($id);
    foreach ($estimate_items as $item) {
        $this->db->insert(db_prefix() . 'estimate_revision_items', [
            ...
            'taxname'           => json_encode(get_estimate_item_taxes($item['id'])),
        ]);
    }

    // Optional: Log revision creation
    $this->log_estimate_activity($id, 'estimate_revised_version_created');
}else{
