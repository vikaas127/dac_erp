...
class Leads_model extends App_Model
{
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['nature_of_work_id'])) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'leads', ['nature_of_work_id' => $data['nature_of_work_id']]);
        }
    ...
    public function get_nature_of_work($id = false) {
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            return $this->db->get(db_prefix() . 'leads_nature_of_work')->row();
        }
    
        $this->db->order_by('name', 'asc');
        $result = $this->db->get(db_prefix() . 'leads_nature_of_work')->result_array();
